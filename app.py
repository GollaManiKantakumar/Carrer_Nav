...
def update_profile(current_user):
    ...
    try:
        ...
        if username and username != current_user:
            ...
            # Also update username in user_roadmaps table
            try:
                cursor2 = conn.cursor()
                cursor2.execute('UPDATE user_roadmaps SET username=? WHERE username=?', (username, current_user))
                conn.commit()
            except sqlite3.Error:
                pass
        
        response_data = {'message': 'Profile updated successfully', 'resume_filename': resume_filename}
        if new_token:
    ...
    if file and file.filename.endswith('.pdf'):
        ...
        try:
            ...
            try:
                ...
                pass
            print("AI Processing HTTP Error:", err_msg)
            if os.path.exists(filepath):
                os.remove(filepath)
            return jsonify({'message': err_msg, 'error': str(e)}), 500
        except Exception as e:
...
def chat_proxy(current_user):
    ...
    try:
        ...
        # Convert frontend message syntax to Gemini API format
        gemini_contents = []
        ...
        # Pass through the exact status from Gemini so frontend knows if it's rate limited
        if not api_res.ok:
            return jsonify(api_res.json()), api_res.status_code
