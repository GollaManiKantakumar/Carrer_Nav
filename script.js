...
async function sendChat() {
    ...
    try {
        ...
        if (!resp.ok) {
            throw new Error(data.message || (data.error && data.error.message) || 'API error');
        }
        body.innerHTML += `<div style="margin-bottom:10px; color:var(--p-brand)"><b>AI:</b> ${data.candidates[0].content.parts[0].text}</div>`;
        body.scrollTop = body.scrollHeight;
    } catch(e) { body.innerHTML += `<p style="color:red">API Error: ${e.message}</p>`; }
}
...
// Update navigateTo to handle profile data refresh and dynamic roadmap selection
...
navigateTo = function(viewId) {
    ...
    if(viewId === 'explore') {
        ...
        // exploreRoleOverride takes priority (set by viewRoleRoadmap), then user's target_role
        const targetRole = window.exploreRoleOverride || user.target_role;
        window.exploreRoleOverride = null; // clear after use
        const filterCard = document.querySelector('#page-explore .form-card-v5');
        const select = document.getElementById('explore-role-select');

        // Remove any existing profile-role badge
        const existingBadge = document.getElementById('explore-profile-badge');
        if(existingBadge) existingBadge.remove();

        if(targetRole && select) {
            const matchingOption = Array.from(select.options).find(o => o.value === targetRole);
            if(matchingOption) {
                // Hide the dropdown filter card
                if(filterCard) filterCard.style.display = 'none';

                // Inject a styled badge showing the active role
                const badge = document.createElement('div');
                badge.id = 'explore-profile-badge';
                badge.style.cssText = 'text-align:center; margin-bottom:25px; padding:18px 30px; background:rgba(139,92,246,0.12); border:1px solid rgba(139,92,246,0.35); border-radius:14px; display:flex; align-items:center; justify-content:center; gap:14px;';
                badge.innerHTML = `
                    <i class="fas fa-crosshairs" style="color:#a78bfa; font-size:1.4rem;"></i>
                    <span style="color:#e4e4e7; font-size:1.05rem;">Your Active Profile Roadmap:</span>
                    <span style="background:linear-gradient(135deg,#8b5cf6,#6366f1); color:white; padding:6px 18px; border-radius:20px; font-weight:700; font-size:1rem; letter-spacing:0.5px;">${targetRole}</span>
                `;
                ...
                setTimeout(() => renderMermaidGraph(targetRole), 100);
                return;
            }
        }

        // No profiled role — show dropdown normally
        if(filterCard) filterCard.style.display = '';
        const roleToRender = (select && select.value) ? select.value : 'Data Analyst';
        setTimeout(() => renderMermaidGraph(roleToRender), 100);
    }
};
...
if (editForm) {
    editForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                // If username changed, backend returns a new token
                if (data.token) {
            ...
            grid.innerHTML = history.map(h => {
                ...
                <div class="form-card-v5" style="padding:25px; transition:all 0.3s; cursor:pointer;" onmouseover="this.style.borderColor='var(--p-brand)'; this.style.transform='translateY(-5px)';" onmouseout="this.style.borderColor='var(--border-subtle)'; this.style.transform='translateY(0)'">
                    ...
                    <div style="display:flex; gap:10px; margin-top:0;">
                        <button class="btn-submit-v5" style="flex:1; padding:12px; font-size:0.9rem; background:linear-gradient(135deg, #18181b, #27272a); border:1px solid rgba(255,255,255,0.1);" onclick="activateHistoricalPath(${h.id})">
                            <i class="fas fa-play-circle" style="color:#8b5cf6;"></i> Mount Trajectory
                        </button>
                        <button class="btn-submit-v5" style="flex:1; padding:12px; font-size:0.9rem; background:linear-gradient(135deg, rgba(99,102,241,0.2), rgba(139,92,246,0.2)); border:1px solid rgba(139,92,246,0.4);" onclick="viewRoleRoadmap('${h.target_role.replace(/'/g, "\\'")}')"> 
                            <i class="fas fa-project-diagram" style="color:#a78bfa;"></i> View Roadmap
                        </button>
                    </div>
                </div>
                `;
...
function viewRoleRoadmap(role) {
    window.exploreRoleOverride = role;
    navigateTo('explore');
}
